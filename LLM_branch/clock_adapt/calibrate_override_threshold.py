...
def fit_fold(rows, train_idx, args, fold_id):
    fold_seed = int(args.seed) + 1009 * int(fold_id)
    ...
    for _ in range(args.epochs):
        order = torch.randperm(len(train_idx), generator=generator).tolist()
        for j in order:
            i = train_idx[j]
...
def main():
    ...
    for t in candidate_thresholds:
        ...
        harmful_fp=sum(int(pred and not r['y'] and r['regret_override']>0.0) for pred,r in zip(do,oof))/max(1,len(negatives))
        sweep.append({'threshold':t,'score':score,'macro_regret':macro,'p90_regret':p90,'raw_false_override_rate':raw_fp,'harmful_false_override_rate':harmful_fp,'override_fraction':sum(do)/max(1,len(do))})
        if harmful_fp<=a.max_false_override_rate:
            key=(score,harmful_fp,-t)
            if best is None or key<best[0]: best=(key,t,raw_fp,harmful_fp,score,macro,p90)
    selected_score=float(best[4] if best else fast_score); improvement=float(fast_score-selected_score)
