...
def select_switch_threshold(rows, folds=5):
    """Family-grouped CV threshold selection; minimizes held-out QoR regret."""
    families = sorted({str(e.get('case', {}).get('family', '')) for e in rows})
    folds = max(1, min(int(folds), len(families)))
    candidates = sorted({0.0, .01, .02, .05, .10, .20, .30, .50})
    best = (float('inf'), .05)
    for threshold in candidates:
        losses=[]
        for fold in range(folds):
            held={f for i,f in enumerate(families) if i % folds == fold}
            for e in rows:
                if str(e.get('case',{}).get('family','')) not in held: continue
                target, fast=baseline_target(e); candidate=int(target.argmin()); selected=candidate if float(target[candidate]) < -threshold else fast
                losses.append(float(target[selected]))
...
def baseline_target(e):
    ...
    return torch.log(qor / qor[fast_idx].clamp_min(1e-12)), fast_idx
    ...
    for epoch in range(a.epochs):
        ...
        with torch.no_grad():
            for e in val:
                ...
                hard_regrets.append(float(target[selected]))
