...
def main():
    ...
    for epoch in range(a.epochs):
        ...
        if score < best: best=score; bad=0; Path(a.output_dir).mkdir(parents=True,exist_ok=True); torch.save({'model':model.state_dict(),'feature_dim':dim,'mem_dim':128,'context_dim':9,'hidden_dim':a.hidden_dim,'dropout':a.dropout,'clock_menu':train[0]['clocks'],'class_weights':class_weights.tolist(),'feature_schema':'memory_attention+budget_bram_dsp_ff_lut+capacity_log+clock_log2_v2','best_val_loss':best},Path(a.output_dir)/'selector.pt')
        else: bad += 1
