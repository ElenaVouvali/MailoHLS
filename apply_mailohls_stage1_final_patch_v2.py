...
def main():
    ...
    try:
        ...
        # Validate only the files this patch edits. Repository-wide
        # `git diff --check` can fail on unrelated tracked experiment logs
        # (for example Checkpoints/.../log.txt) and should not invalidate
        # an otherwise correct Stage-1 patch.
        subprocess.run(
            [
                "git", "diff", "--check", "--",
