...
# Clean comparison against absolute_direct_rank.  Only the grouped sampler
# geometry changes: 8 kernels x 8 points, while the optimizer batch remains 64.
SPLIT="mailohls_runs/mailohls_final_family_split_s123.json"
EXPERIMENT="${EXPERIMENT:-gnn_final_absolute_rankdense_s123}"
...
python -u GNN_branch/main_GNN.py \
  ...
  --smooth_l1_beta 0.5 \
  --standardize_targets --qor_output_init_scale 1.0 \
  --kernel_balanced_loss --kernel_grouped_sampling \
  --kernels_per_batch 8 --points_per_kernel 8 \
  --samples_per_kernel_per_epoch 128 --batch_size 64 --grad_accum_steps 1 \
  --rank_aux_weight 0.05 --pairwise_delta_weight 0 --rank_tie_relative 0.05 \
  --resource_aux_weight 0.10 --resource_budget_bank "${BUDGET_BANK}" \
  --resource_budget_count 16 --resource_budget_min_fraction 0.05 \
  --resource_boundary_tolerance 0.02 \
  --checkpoint_objective qualified_lexicographic --min_rank_tau 0.20 \
