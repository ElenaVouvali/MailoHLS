#include "network.h"
#include "weight_definitions.h"
#include "tanh.h"

l_quantized_type ReLU(l_quantized_type res)
{
	if (res < 0)
		return 0;

	return res;
}

l_quantized_type tanh(l_quantized_type res)
{
	if (res >= 2)
		return 1;
	else if (res < -2)
		return -1;
	else
	{
		ap_int <BITS+2> i = res.range();			//prepare result to match tanh value
		return tanh_vals[(BITS_EXP/2) + i.to_int()];
	}
}

void forward_propagation(float *x, float *y)
{
/*L1:*/	quantized_type xbuf[N1];
#pragma HLS array_partition variable=xbuf type=auto{_ARRAY_T_L1} factor=auto{_ARRAY_F_L1} dim=auto{_ARRAY_D_L1}
/*L2:*/	l_quantized_type layer_1_out[M1];
#pragma HLS array_partition variable=layer_1_out type=auto{_ARRAY_T_L2} factor=auto{_ARRAY_F_L2} dim=auto{_ARRAY_D_L2}
/*L3:*/	l_quantized_type layer_2_out[M2];
#pragma HLS array_partition variable=layer_2_out type=auto{_ARRAY_T_L3} factor=auto{_ARRAY_F_L3} dim=auto{_ARRAY_D_L3}


	// limit resources to max DSP number of Zybo - do not change
	// #pragma HLS ALLOCATION instances=mul limit=80 operation


/*L4:*/	for (int i=0; i<N1; i++)
#pragma HLS pipeline II=auto{_PIPE_L4}
#pragma HLS unroll factor=auto{_UNROLL_L4}
	{
		xbuf[i] = x[i];
	}

	// Layer 1
/*L5:*/	for(int i=0; i<N1; i++)
#pragma HLS pipeline II=auto{_PIPE_L5}
#pragma HLS unroll factor=auto{_UNROLL_L5}
	{
/*L6:*/	for(int j=0; j<M1; j++)
#pragma HLS pipeline II=auto{_PIPE_L6}
#pragma HLS unroll factor=auto{_UNROLL_L6}
		{
			l_quantized_type last = (i==0) ? (l_quantized_type) 0 : layer_1_out[j];
			quantized_type term = xbuf[i] * W1[i][j];
			layer_1_out[j] = last + term;
		}
	}

/*L7:*/	for(int i=0; i<M1; i++)
#pragma HLS pipeline II=auto{_PIPE_L7}
#pragma HLS unroll factor=auto{_UNROLL_L7}
	{
		layer_1_out[i] = ReLU(layer_1_out[i]);
	}

	// Layer 2
/*L8:*/	for(int i=0; i<M2; i++)
#pragma HLS pipeline II=auto{_PIPE_L8}
#pragma HLS unroll factor=auto{_UNROLL_L8}
	{
		l_quantized_type result = 0;
/*L9:*/	for(int j=0; j<N2; j++)
#pragma HLS pipeline II=auto{_PIPE_L9}
#pragma HLS unroll factor=auto{_UNROLL_L9}
		{
			l_quantized_type term = layer_1_out[j] * W2[j][i];
			result += term;
		}
		layer_2_out[i] = ReLU(result);
	}

	// Layer 3
/*L10:*/	for(int i=0; i<M3; i++)
#pragma HLS pipeline II=auto{_PIPE_L10}
#pragma HLS unroll factor=auto{_UNROLL_L10}
	{
		l_quantized_type result = 0;
/*L11:*/	for(int j=0; j<N3; j++)
#pragma HLS pipeline II=auto{_PIPE_L11}
#pragma HLS unroll factor=auto{_UNROLL_L11}
		{
			l_quantized_type term = layer_2_out[j] * W3[j][i];
			result += term;
		}
		y[i] = tanh(result).to_float();
	}
}
