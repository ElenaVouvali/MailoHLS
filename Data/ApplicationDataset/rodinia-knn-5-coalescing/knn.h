...
#ifdef __VIVADO_HLS__
  #include "ap_int.h"
  const int DWIDTH = 512;
  #define INTERFACE_WIDTH ap_uint<DWIDTH>
  const int WIDTH_FACTOR = DWIDTH / 32;
  // HLS: real bit-range operations
  #define GET_RANGE(x, hi, lo)           (x).range((hi), (lo))
  #define SET_RANGE(x, hi, lo, val)      (x).range((hi), (lo)) = (val)

#else
  #include <stdint.h>
  const int DWIDTH = 512;
  typedef uint32_t INTERFACE_WIDTH;
  const int WIDTH_FACTOR = DWIDTH / 32;
  #define GET_RANGE(x, hi, lo)           (x)
  #define SET_RANGE(x, hi, lo, val)      do { (x) = (val); } while(0)
#endif
