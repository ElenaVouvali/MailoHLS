...
def initialize_qor_heads_conservatively(heads, scale=0.1):
...
def reference_delta_head_response(head, full_input, neutral_input, head_mode):
    """Evaluate either the anchored or production-style direct delta head."""
    if head_mode == 'anchored':
        return anchored_head_response(head, full_input, neutral_input)
    if head_mode == 'direct':
        return head(full_input)
    raise ValueError(f'Unknown reference-delta head mode: {head_mode!r}')


def reference_delta_loss_target(target, mean, std, *, standardized, head_mode):
    """Transform a measured log2 delta under the selected response head."""
    if not standardized:
        return target
    if head_mode == 'anchored':
        return target / std
...
def inverse_reference_delta(output, mean, std, *, standardized, head_mode):
    """Map a response-head output back to a measured log2 QoR delta."""
    if not standardized:
        return output
    if head_mode == 'anchored':
        return output * std
...
class Net(nn.Module):
    def __init__(self, in_channels, edge_dim = 0, init_pragma_dict = None,
          ...
          initialize_qor_heads_conservatively(
              self.MLPs, float(getattr(FLAGS, 'qor_output_init_scale', 0.1))
          )
          if self.decompose_targets:
              ...
              initialize_qor_heads_conservatively(
                  self.center_MLPs,
                  float(getattr(FLAGS, 'qor_output_init_scale', 0.1)),
              )
    ...
    def forward(self, data):
        ...
        if self.MLP_version == 'multi_obj':
            if self.reference_delta:
                out_MLPs = reference_delta_head_response(
                    self.MLPs,
                    out_embed,
                    neutral_embed,
                    self.reference_delta_head,
                )
            else:
        ...
        for target_name in self.target_list:
            ...
            if self.task == 'regression':
                ...
                loss_target = (
                    reference_delta_loss_target(
                        target,
                        mean,
                        std,
                        standardized=bool(self.targets_standardized.item()),
                        head_mode=self.reference_delta_head,
                    )
                    if self.reference_delta
                    else (
                        (target - mean) / std
