...
class MLIRGraphBuilder:
    ...
    * action passes attach each source "Lk" loop/array action exactly once.
    ...
    def _array_argument_node(
        ...
        # Shape is a compatibility filter, not a source identity.  Attempt
        # exact rooted source-access matching before any shape-only fallback,
        # even when there is only one allocation with the requested shape.
        # This keeps production action mapping source-grounded without
        # changing the graph representation.
        if shape_candidates and spec.source_use_lines:
            ...
            if best_score > 0 and len(best) == 1:
                ...
                return (
                    ...
                    node,
                    record.block_id,
                    # The physical root was selected by exact MLIR source
                    # access locations, so use the validator's existing exact
                    # source-location category.  Function/shape are filters,
